...
class _MyHomePageState extends State<CategoryHomeScreen> {
  ...
  Future<void> getdetailsProductsAfterTap(link2) async {
    log("calling details");
    //String biscuitSweetsURl = "https://test.protidin.com.bd/api/v2/products/category/46";

    final response7 = await get(Uri.parse(link2), headers: {"Accept": "application/json"});

    var detailsDataMap=jsonDecode(response7.body);

    if(detailsDataMap["success"]==true){
      //log("category data after tap $biscuitSweetsDataMap");

      setState(() {
        var biscuitSweetsDataModel= Details.fromJson(detailsDataMap);
        detailsProducts=biscuitSweetsDataModel.data.cast<Details>();
      });
      log("categoryProducts data length ${detailsProducts.length}");
